...
> **In progress: a real multi-fidelity DOE campaign (§8.7, full writeup next session), replacing the single-variable washout sweep above with proper design-space coverage.** A genuine 3-variable design space (twist at mid-span, twist at tip, angle of attack) was sampled with a 200-point Latin Hypercube DOE, evaluated with a real, fast, already-validated VLM solver (OpenAeroStruct, Build 1b) — not a lookup table. All 4 planned candidates from that screening have now been refined with real, full-fidelity SU2 solves (2500-iteration, fixed-CFL, the same rigor established in §2/§8.3):
>
> | Candidate | VLM-predicted L/D | Real SU2-converged L/D | Overprediction |
> |---|---|---|---|
> | rank0 (top cluster) | 25.08 | **13.60** | 1.84× |
> | rank1 (top cluster) | 25.07 | **13.88** | 1.81× |
> | rank2 (top cluster) | 25.00 | **13.82** | 1.81× |
> | rank5 (diverse candidate) | 24.34 | **19.72** | 1.23× |
>
> Two real findings survive verification: (1) VLM systematically overestimates L/D at this Mach 0.85 condition (no shock-capturing — a genuine, honest limit of cheap-fidelity screening at transonic conditions, not noise), and (2) **VLM's ranking did not transfer to reality** — its "5th-best" candidate (rank5) is the real best of all 4 once wave drag is accounted for, beating the VLM top-3 cluster by a wide margin. This is itself an important, reportable finding about multi-fidelity methodology: cheap screening can rank candidates in the wrong order at transonic conditions, not just get the absolute number wrong. Raw data: [`doe_vlm_screening_log.csv`](aerostructural_su2_mystran/sample_logs/doe_vlm_screening_log.csv) (200 real VLM points), [`doe_vlm_top_candidates.csv`](aerostructural_su2_mystran/sample_logs/doe_vlm_top_candidates.csv), [`doe_su2_refine_log.csv`](aerostructural_su2_mystran/sample_logs/doe_su2_refine_log.csv) (all 4 SU2 refinements complete). Next session: full §8.7 writeup, comparison against the earlier −2° single-variable result, and a final structural resize on rank5 (the real best candidate) if it holds up as the final design choice.
