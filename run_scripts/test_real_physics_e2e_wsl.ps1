...
$flags = @(
    "--config '$configWsl'",
    "--coupling-iters $CouplingIters",
    "--optimizer-iters $OptimizerIters"
)
if ($SkipOptimize) { $flags += "--skip-optimize" }
if ($RequireCuda) { $flags += "--require-cuda" }
...
$cmd = "source /opt/miniforge3/etc/profile.d/conda.sh; " +
       ...
       $cudaPrefix +
       "cd '$repoWsl'; " +
       "python run_scripts/test_real_physics_e2e.py $flagText"
