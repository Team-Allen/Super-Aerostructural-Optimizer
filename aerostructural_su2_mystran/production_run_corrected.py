"""
Build 2 production run on the CORRECTED real aircraft wing shape: symmetric
(~7% t/c) airfoil cross-section, real 3.0m semi-span, real 2.688m/0.627m
taper, real 26.8-degree sweep -- all measured directly from the real CAD
reference (Reference Docs/MYSTRAN/PipeLine_Building/Wing_Surface_Cleaned.stp)
via gmsh's OCC kernel and scaled 12.824x to the real aircraft (semi-span 3.0m).

This replaces production_run_realshape.py, which used an assumed 6m-semispan,
4.5m/1.5m-chord, 35-degree-sweep, cambered-NACA2412 planform that was never
checked against the CAD file -- caught after the resulting 72kg mass was
flagged as implausible given the real aircraft's much smaller actual scale.

Same proven VAM-FSD + discrete ply-count architecture and full per-iteration
logging/archiving as production_run.py / production_run_realshape.py.
"""
...
WORKDIR = os.path.join(HERE, "production_run_final")
...
SU2_SURFACE_CSV = os.path.join(HERE, "corrected_mesh_test5", "surface_flow.csv")
...
def main():
    ...
    print(f"Corrected real aircraft wing shell: {info['n_nodes']} nodes, {info['n_elements']} elements, "
          f"{info['n_zones']} zones")
    shutil.copy(BDF_PATH, TEMPLATE_BDF)

    # align=False: this mesh shares the SU2 mesh's exact coordinate frame
    # (same symmetric-section + sweep/taper formula), so no coordinate
    # shift is applied.
